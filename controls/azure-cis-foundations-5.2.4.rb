control "azure-cis-9.2-control-5.2.4" do
  title "Ensure that Activity Log Alert exists for Create or Update Network
Security Group Rule"
  desc  "Create an activity log alert for the Create or Update Network Security
Group Rule event."
  desc  "rationale", "Monitoring for Create or Update Network Security Group
Rule events gives insight into network access changes and may reduce the time
it takes to detect suspicious activity."
  desc  "check", "
    **Azure Console**
...
Administrative Activity Log \"Create or Update Security Rule
...
and
.equals==\"microsoft.network/networksecuritygroups/securityrules/write\"),enabled:.properties.enabled}'
    ```

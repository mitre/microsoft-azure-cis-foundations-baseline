...
outside of the cloud environment. User profiles contain private information
...
third parties without requiring any further consent from the user."
  desc  "check", "
    **Azure Console**
    ...
    4. Ensure that `Users can consent to apps accessing company data on their
behalf` is set to `No`
...
recommendation.*
  ...
  desc  "fix", "
    **Azure Console**
    ...
    2. Go to `Users and group`
    3. Go to `User settings`
    4. Set `Users can consent to apps accessing company data on their behalf`
to `No`
  "
  impact 0.7
  tag severity: "high"
